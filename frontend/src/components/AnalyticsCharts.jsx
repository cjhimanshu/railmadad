...
import {
    FaChartPie, FaExclamationTriangle, FaUsers, FaClock,
    FaSmile, FaMeh, FaFrown, FaFire, FaArrowUp,
} from 'react-icons/fa';
...
const CATEGORY_COLORS = [
    ...
    '#F59E0B','#F97316','#EF4444','#EC4899','#8B5CF6','#84CC16',
];

// ─── Custom donut centre label ──────────────────────────────────────────────────
const DonutLabel = ({ viewBox, total, label }) => {
    const { cx, cy } = viewBox;
    return (
        <>
            <text x={cx} y={cy - 8} textAnchor="middle" dominantBaseline="middle"
                style={{ fontSize: 28, fontWeight: 800, fill: '#1E293B' }}>
                {total}
            </text>
            <text x={cx} y={cy + 16} textAnchor="middle" dominantBaseline="middle"
                style={{ fontSize: 11, fill: '#94A3B8', letterSpacing: 1, textTransform: 'uppercase' }}>
                {label}
            </text>
        </>
    );
...
const GlassPanel = ({ children, className = '' }) => (
    <div className={`bg-white/80 backdrop-blur-sm border border-white/60 rounded-2xl shadow-lg ${className}`}>
        {children}
    </div>
);
...
const AnalyticsCharts = ({ analytics }) => {
    ...
    const statusData   = Object.entries(analytics.complaintsByStatus || {}).map(([k, v]) => ({ name: k, value: v, color: STATUS_COLORS[k] || '#94A3B8' }));
    const priorityData = Object.entries(analytics.complaintsByPriority || {}).map(([k, v]) => ({ name: k, value: v, color: PRIORITY_COLORS[k] || '#94A3B8' }));
    const sentimentData= Object.entries(analytics.complaintsBySentiment || {}).map(([k, v]) => ({ name: k, value: v, color: SENTIMENT_COLORS[k] || '#94A3B8' }));
    ...
    const KPI_CARDS = [
        {
            ...
            grad: 'from-blue-500 to-indigo-600', icon: FaChartPie, light: 'bg-blue-50 text-blue-600',
        },
        {
            label: 'Registered Users', value: analytics.totalUsers,
            sub: 'Active accounts',
            grad: 'from-violet-500 to-purple-600', icon: FaUsers, light: 'bg-violet-50 text-violet-600',
        },
        {
            label: 'Avg Resolution', value: `${analytics.avgResolutionTimeHours}h`,
            sub: 'Mean time to close',
            grad: 'from-teal-500 to-emerald-600', icon: FaClock, light: 'bg-teal-50 text-teal-600',
        },
        {
            label: 'This Week', value: analytics.recentComplaints,
            sub: `${recentPct}% of all time`,
            grad: 'from-orange-500 to-rose-500', icon: FaArrowUp, light: 'bg-orange-50 text-orange-600',
        },
    ];
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <ResponsiveContainer width="100%" height={180}>
                        <PieChart>
                            <Pie data={statusData} cx="50%" cy="50%"
                                ...
                                <DonutLabel total={totalStatus} label="total" />
                            </Pie>
                            <Tooltip content={<CustomTooltip />} />
                        </PieChart>
                    </ResponsiveContainer>
                    <div className="mt-2 divide-y divide-gray-50">
                        {statusData.map(d => <LegendPill key={d.name} color={d.color} label={d.name} value={d.value} />)}
                    </div>
                </GlassPanel>
                    ...
                    <ResponsiveContainer width="100%" height={180}>
                        <PieChart>
                            <Pie data={priorityData} cx="50%" cy="50%"
                                ...
                                <DonutLabel total={totalPriority} label="total" />
                            </Pie>
                            <Tooltip content={<CustomTooltip />} />
                        </PieChart>
                    </ResponsiveContainer>
                    <div className="mt-2 divide-y divide-gray-50">
                        {priorityData.map(d => <LegendPill key={d.name} color={d.color} label={d.name} value={d.value} />)}
                    </div>
                </GlassPanel>
                ...
                <GlassPanel className="p-6">
                    ...
                    <div className="mt-2 divide-y divide-gray-50">
                        {sentimentData.map(d => {
                            const Icon = d.name === 'positive' ? FaSmile : d.name === 'negative' ? FaFrown : FaMeh;
                            return (
